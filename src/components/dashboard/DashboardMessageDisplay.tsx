...
import { DashboardMessageProps, SentimentResult } from "@/types/types";
...
export const DashboardMessageDisplay: React.FC<DashboardMessageProps> = ({
  ...
  const extractConversationalText = (content: unknown): string => {
    ...
    if (content && typeof content === "object") {
      ...
      if (
        "response" in content &&
        typeof (content as { response?: unknown }).response === "string"
      ) {
      ...
      if (
        ...
        typeof (content as { conversation?: unknown }).conversation ===
          "object" &&
        (content as { conversation?: unknown }).conversation
      ) {
        const conversation = (
          content as { conversation: { generated_responses?: unknown } }
        ).conversation;
        if (
          "generated_responses" in conversation &&
          Array.isArray(conversation.generated_responses)
        ) {
          const responses = conversation.generated_responses;
          if (
            responses.length > 0 &&
            typeof responses[responses.length - 1] === "string"
          ) {
    ...
    switch (modelCategory) {
      ...
      case "creative":
        ...
        if (typeof message.content === "string") {
          ...
          typeof (message.content as { generated_text?: string })
            .generated_text === "string"
        ) {
          // Extract text from the 'generated_text' field
          creativeContent = (message.content as { generated_text: string })
            .generated_text;
        } else if (
          message.content &&
          typeof message.content === "object" &&
          "response" in message.content && // Added check for 'response' field
          typeof (message.content as { response?: string }).response ===
            "string"
        ) {
